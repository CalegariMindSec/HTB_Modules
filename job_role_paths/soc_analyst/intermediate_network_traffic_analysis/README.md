...
```bash
Kl3gari@htb[/htb]$ wget -O file.zip 'https://academy.hackthebox.com/storage/resources/pcap_files.zip' && mkdir tempdir && unzip file.zip -d tempdir && mkdir -p pcaps && mv tempdir/Intermediate_Network_Traffic_Analysis/* pcaps/ && rm -r tempdir file.zip
...
```shellsession
        shellsession
Kl3gari@htb[/htb]$ sudo apt install tcpdump -y
```
...
```shellsession
        shellsession
Kl3gari@htb[/htb]$ sudo tcpdump -i eth0 -w filename.pcapng
```
...
```shellsession
        shellsession
Kl3gari@htb[/htb]$ wireshark ARP_Spoof.pcapng
```
...
```shellsession
        shellsession
Kl3gari@htb[/htb]$ arp -a | grep 50:eb:f6:ec:0e:7f

? (192.168.10.4) at 50:eb:f6:ec:0e:7f [ether] on eth0
        shellsession
Kl3gari@htb[/htb]$ arp -a | grep 08:00:27:53:0c:ba
...
> **Answer:** 507
>
> Wireshark filter: eth.src == 08:00:27:53:0c:ba and arp.opcode == 1
...
1 - Inspect the ARP_Poison.pcapng file, part of  this module's resources, and submit the first MAC address that was  linked with the IP  as your answer.

> **Answer:** 2c:30:33:e2:d5:c3
>
> Wireshark filter: arp.src.proto_ipv4 == 192.168.10.1 || arp.dst.proto_ipv4 == 192.168.10.1
...
```shellsession
        shellsession
Kl3gari@htb[/htb]$ sudo airmon-ng start wlan0
...
```shellsession
        shellsession
Kl3gari@htb[/htb]$ sudo ifconfig wlan0 down
Kl3gari@htb[/htb]$ sudo iwconfig wlan0 mode monitor
Kl3gari@htb[/htb]$ sudo ifconfig wlan0 up
```
...
```shellsession
        shellsession
Kl3gari@htb[/htb]$ sudo airodump-ng -c 4 --bssid F8:14:FE:4D:E6:F1 wlan0 -w raw
...
```shellsession
        shellsession
Kl3gari@htb[/htb]$ sudo wireshark deauthandbadauth.cap
```
...
> **Answer:** 14592
>
> Wireshark filter: (wlan.bssid == f8:14:fe:4d:e6:f1) and (wlan.fc.type == 00) and (wlan.fc.type_subtype == 12)
...
```shellsession
        shellsession
Kl3gari@htb[/htb]$ sudo airodump-ng -c 4 --essid HTB-Wireless wlan0 -w raw
...
>
> Wireshark filter: (wlan.bssid == F8:14:FE:4D:E6:F2)
...
```shellsession
        shellsession
Kl3gari@htb[/htb]$ wireshark nmap_frag_fw_bypass.pcapng
```
...
```shellsession
        shellsession
Kl3gari@htb[/htb]$ nmap -f 10 <host ip>
```
...
> **Answer:** 66535
>
> Wireshark filter: frame.coloring_rule.name contains "RST"
...
> **Answer:** 1
>
> Wireshark filter: _ws.col.protocol == "ICMP"
...
> **Answer:** 429
>
> Wireshark filter: _ws.col.info contains "ACK"
...
1 - Inspect the TCP-hijacking.pcap file, part of  this module's resources, and enter the username that has been used  through the telnet protocol as your answer.

> **Answer:** administrator
>
> Note: Look for packets with command line 
...
```shellsession
        shellsession
Kl3gari@htb[/htb]$ echo 'VGhpcyBpcyBhIHNlY3VyZSBrZXk6IEtleTEyMzQ1Njc4OQo=' | base64 -d
```
...
```shellsession
        shellsession
Kl3gari@htb[/htb]$ cat access.log | grep "192.168.10.5"
...
```shellsession
        shellsession
Kl3gari@htb[/htb]$ cat access.log | awk '$1 == "192.168.10.5"'
...
> **Answer:** 204
>
> Wireshark Filter: http.request.method == "GET" and tcp.dstport == 80
...
1 - Inspect the first packet of the  XSS_Simple.pcapng file, part of this module's resources, and enter the  cookie value that was exfiltrated as your answer.
...
| `Key Exchange`               | `ClientDHPrivateKey``ClientDHPublicKey = DH_KeyGeneration(ClientDHPrivateKey)``ClientKeyExchange = { ClientDHPublicKey }``ServerDHPrivateKey``ServerDHPublicKey = DH_KeyGeneration(ServerDHPrivateKey)``ServerKeyExchange = { ServerDHPublicKey }` |
| `Premaster Secret`           | `PremasterSecret = DH_KeyAgreement(ServerDHPublicKey, ClientDHPrivateKey)``PremasterSecret = DH_KeyAgreement(ClientDHPublicKey, ServerDHPrivateKey)` |
...
| `Extraction of Session Keys` | `ClientWriteMACKey = First N bytes of KeyBlock``ServerWriteMACKey = Next N bytes of KeyBlock``ClientWriteKey = Next N bytes of KeyBlock``ServerWriteKey = Next N bytes of KeyBlock``ClientWriteIV = Next N bytes of KeyBlock``ServerWriteIV = Next N bytes of KeyBlock` |
...
```shellsession
        shellsession
Kl3gari@htb[/htb]$ echo 'VTBaU1EyVXhaSFprVjNocldETnNkbVJXT1cxaU0wb3pXVmhLYTFneU1XeFlNMUp2WVZoT1ptTklTbXhrU0ZJMVdETkNjMXBYUm5wYQpXREJMQ2c9PQo=' | base64 -d 
...
```shellsession
        shellsession
Kl3gari@htb[/htb]$ echo 'VTBaU1EyVXhaSFprVjNocldETnNkbVJXT1cxaU0wb3pXVmhLYTFneU1XeFlNMUp2WVZoT1ptTklTbXhrU0ZJMVdETkNjMXBYUm5wYQpXREJMQ2c9PQo=' | base64 -d | base64 -d | base64 -d
```
...
1 - Enter the decoded value of the triple  base64-encoded string that was mentioned in this section as your answer. Answer format: HTB{___}
...
1 - Inspect the telnet_tunneling_ipv6.pcapng file,  part of this module's resources, and enter the hidden flag as your  answer. Answer format: HTB(___) (Replace all spaces with underscores)
...
1 - Inspect the funky_dns.pcap file, part of this  module's resources, and enter the related attack as your answer. Answer  format: "DNS Flooding", "DNS Amplification", "DNS Tunneling"

> **Answer:** DNS Tunneling

2 - Inspect the funky_icmp.pcap file, part of this  module's resources, and enter the related attack as your answer. Answer  format: "ICMP Flooding", "ICMP Tunneling", "ICMP SMURF Attack"
